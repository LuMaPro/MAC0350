...
def buscar_alunos(busca: str, pagina: int, limite: int = 10):
    with Session(engine) as session:
        offset = (pagina - 1) * limite
        query = select(Aluno).where(col(Aluno.nome).contains(busca)).order_by(Aluno.nome).offset(offset).limit(limite)
...
def lista(request: Request, busca: str | None = '', pagina: int = 1):
    limite = 10
    alunos = buscar_alunos(busca, pagina, limite)
    
    tem_proxima = len(alunos) == limite
...
def criar_aluno(nome: str = Form(...)):
    with Session(engine) as session:
        ...
        return HTMLResponse(content=f"<p style='color: green;'>O(a) aluno(a) {novo_aluno.nome} foi registrado(a)!</p>")
...
def deletar_aluno(id: int = Form(...)): 
    with Session(engine) as session:
        ...
        if not aluno:
            return HTMLResponse(content=f"<p style='color: orange;'>Aviso: Nenhum aluno encontrado com o ID {id}.</p>")
            
        session.delete(aluno)
        session.commit()
        return HTMLResponse(content=f"<p style='color: red;'>O(a) aluno(a) {aluno.nome} foi deletado(a)!</p>")
...
def atualizar_aluno(id: int = Form(...), novoNome: str = Form(...)):
    with Session(engine) as session:
        ...
        if not aluno:
            return HTMLResponse(content=f"<p style='color: orange;'>Aviso: Não é possível atualizar. ID {id} não existe.</p>")
            
        nomeAntigo = aluno.nome
        aluno.nome = novoNome
        session.commit()
        session.refresh(aluno)
        return HTMLResponse(content=f"<p style='color: blue;'>O(a) aluno(a) {nomeAntigo} foi atualizado(a) para {aluno.nome}!</p>")
